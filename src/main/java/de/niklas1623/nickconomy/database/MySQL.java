...
public class MySQL {
    ...
    public static void createTable() {
        if (isConnected()) {
            try {
                con.createStatement().executeUpdate("CREATE TABLE IF NOT EXISTS player (PID INT NOT NULL AUTO_INCREMENT, Name TEXT NOT NULL, UUID TEXT NOT NULL , PRIMARY KEY (PID))");
                ...
                con.createStatement().executeUpdate("CREATE TABLE IF NOT EXISTS accounttyp (AtID INT NOT NULL, Typ VARCHAR(64) NOT NULL, PRIMARY KEY(AtID))");
            } catch (SQLException ex) {
                ex.printStackTrace();
            }
        }
